...
impl Relation {
	fn scopes(&self, schemas: &[Schema]) -> Vector<DataType> {
		...
		match self {
			...
			Join { left, kind: JoinKind::Semi, .. } => left.scopes(schemas),
	...
	fn eval(self, source: Relation) -> syn::Relation {
		...
		match source {
			...
			Filter { condition, source } => {
				...
				Rel::lam(scopes, condition * source.app(vars))
			},
			...
			Correlate(left, right) => {
				...
				Rel::lam(scopes, left.app(left_vars) * right.app(right_vars))
			},
			// R(x) union S(y)
			// λx. R(x) + S(x)
			Union(left, right) => {
				...
				Rel::lam(scopes, left.app(vars.clone()) + right.app(vars))
			},
			// R(x) except S(y)
			// λx. R(x) × ¬S(x)
			Except(left, right) => {
			...
			Distinct(source) => {
				...
				Rel::lam(scopes.clone(), UExpr::squash(source.app(vars(lvl, scopes))))
			},
